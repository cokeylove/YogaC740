...
void service_scan(void)
{
	...
	//MARTINY013:add start
	   if (IS_MASK_SET(pDevStatus1,b2DisableKB))
		{
		   F_Service_KEY = 0;   
		   return;
		}
	   if(!Read_LID_SW_IN())  //HEGANGS030:Disable key wake in pad mode
	   {
		   F_Service_KEY = 0;   
		   return;
		}
...
void S4S5_KeyWake(BYTE  ONEKEY_TYPE)//HEGANGS021:one key wake
{
	if(Read_YOGA_ID())
	{
		return;
	}
	if(IS_MASK_SET(ACPI_HOTKEY, b6Cmd_NoShut))//HEGANGS046:Modify the 5VALW power off if the charge in battery Mode is disable
	{
		return;
	}
	if(IS_MASK_CLEAR(OEMControl,b2ONEKEY_ENABLE))//HEGANGS025: Modify onekey battery function
	{
		return;
	}
	switch (SysPowState)
	{
		...
		PowSeqDelay = 0x00;
		uNovoVPCCount = 0;
		ONEKEY_FLAG =ONEKEY_TYPE;
		ONEKEY_TEMP_FLAG=ONEKEY_TYPE;//HEGANGS025: Modify onekey battery function
		RamDebug(0x56);
