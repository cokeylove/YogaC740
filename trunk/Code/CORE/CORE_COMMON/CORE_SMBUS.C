...
const sSMBus code asSMBus[]=
{
	...
	{ &HOCTL_C, &TRASLA_C, &HOCMD_C, &HOSTA_C, &D0REG_C, &D1REG_C,	&HOBDB_C,	&IER2,	&ISR2,	Int_SMBUS2,   &PECERC_C},//72JERRY060//72JERRY074:Enable smbus3 channel.
...
const sResetSMBusS code asResetSMBusS[]=
{
	...
	{ &HOSTA_C, &HOCTL2_C, &SMBPCTL_C, &GPCRF6, &GPCRF7, &GPDRF,	(BIT6+BIT7),   &SMBus3RecoverCunt },//72JERRY060//72JERRY074:Enable smbus3 channel.
...
//72JERRY060£º Start i2c for access PD
//==============================================================================
// Name: I2CCompatibleWrite2Read
// Input: bSMBusCh = 0~2
//        addr = Slave address
//        bDataLength = Max 0xFF
//        *bVar = Data buffer
// Return: 0 = Fail, 1 = Correct Pass
//==============================================================================
BYTE I2CCompatibleWrite(BYTE bSMBusCh,BYTE addr,BYTE bWrLength,XBYTE *bVarWr)
{
	BYTE result;
	BYTE status;
	BYTE error;
	...
	if (bSMBusCh == 0xFF)
	{
		return (FALSE);		// no support device or device busy
	}

	if (CheckSMBusInterfaceCanbeUse(bSMBusCh,SMBus_AccessType_Fucn) == SMBus_CanNotUse)
	{
		return (FALSE);		// no support device or device busy
	}

	*asResetSMBusS[bSMBusCh].SMBusCTL2 |= 0x03;		// Enable SMBus and compatible I2C
	*asSMBus[bSMBusCh].SMBusADR = addr;		// Set device address
	*asSMBus[bSMBusCh].SMBusSTA |= 0xFE;		// Clear error flag

	//=======WRITE ===============================================================//

	*asSMBus[bSMBusCh].SMBusBData = *bVarWr;	// Set data to host block data byte reg
	*asSMBus[bSMBusCh].SMBusCTL = 0x5C;		// Start send (None interrupt)
	...
	_nop_();
	_nop_();
	TH1 = Timer_40ms >> 8;		// Set timer1 counter 26ms
	TL1 = Timer_40ms;		// Set timer1 counter 26ms
	TF1 = 0;			// Clear overflow flag
	TR1 = 1;			// Enable timer1

	while (!TF1)			// Not time out
	{
		status = *asSMBus[bSMBusCh].SMBusSTA;	// Read SMBus Host Status
		...
		if (IS_MASK_SET(status,(HOSTA_BDS + HOSTA_DVER + HOSTA_BSER + HOSTA_FAIL + HOSTA_NACK + HOSTA_TMOE)))
		{
			...
			{
				bWrLength--;
				bVarWr++;		// point to next address of variable

				if (bWrLength != 0x00)
				{
					*asSMBus[bSMBusCh].SMBusBData = *bVarWr;
					*asSMBus[bSMBusCh].SMBusSTA = 0xFE;	// clear bits
				}
				else			// write block transaction done
				{
					TR1 = 0;	// disable timer1
	...
	*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xFD;	// Disable I2C_EN
	return result;
}
...
// Return: 0 = Fail, 1 = Correct Pass
//==============================================================================
BYTE I2CCompatibleWrite2Read(BYTE bSMBusCh, BYTE addr, WORD bWrLength, XBYTE *bVarWr, WORD bRdLength, XBYTE *bVarRd)
{
	BYTE result;
	BYTE status;
	BYTE error;
	...
	if (bWrLength)
	{
		...
		*asResetSMBusS[bSMBusCh].SMBusCTL2 |= 0x03;							// Enable SMBus and compatible I2C
		*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xEF;							// Disable SMBus Data Time out Enable
		*asSMBus[bSMBusCh].SMBusADR = addr;	// Set device address
		*asSMBus[bSMBusCh].SMBusSTA |= 0xFE;							// Clear error flag
		...
		*asSMBus[bSMBusCh].SMBusCTL = 0x5C;					// Start send (None interrupt)
				...
				else							// Only Byte Done Status bit is set.
				{
					...
					if (bWrLength != 0x00)
					{
						...
						*asSMBus[bSMBusCh].SMBusSTA = 0xFE;	// clear bits
						...
						if ((bWrLength == 1) && (bRdLength == 0))
						{
							SET_MASK(*asSMBus[bSMBusCh].SMBusCTL, HOCTL_LABY);
						}
					}
					else						// write block transaction done
					{
						...
						{
							*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xFD; 	// Disable I2C_EN
							*asSMBus[bSMBusCh].SMBusSTA = 0xFC; 	// clear bits
						}
		...
		if (!bRdLength)
		{
			...
			*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xFD; 	// Disable I2C_EN
			*asResetSMBusS[bSMBusCh].SMBusCTL2 |= 0x10;							// Enable SMBus Data Time out Enable
			*asSMBus[bSMBusCh].SMBusSTA = 0xFE; 	// clear bits

			return result;
		}

		//*asSMBus[bSMBusCh].SMBusADR = (addr | 0x01);
		*asResetSMBusS[bSMBusCh].SMBusCTL2 |= 0x0C;		// Set I2C_SW_EN = 1, Set I2C_SW_WAIT = 1;
		*asSMBus[bSMBusCh].SMBusSTA |= 0x80;		// Clear byte done
		*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xFB;		// CLEAR I2C_SW_WAIT

	}
	else
	{
		*asResetSMBusS[bSMBusCh].SMBusCTL2 |= 0x03;
		*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xEF;							// Disable SMBus Data Time out Enable
		*asSMBus[bSMBusCh].SMBusSTA = 0xFE;
		*asSMBus[bSMBusCh].SMBusADR = (addr | 0x01);
		*asSMBus[bSMBusCh].SMBusCTL = (SMBusEXT | HOCTL_SRT);
	}
	...
	while (!TF1)							// Not time out
	{
		...
		if (IS_MASK_SET(status, (HOSTA_BDS + HOSTA_DVER + HOSTA_BSER + HOSTA_FAIL + HOSTA_NACK + HOSTA_TMOE)))
		{
			...
			{
				*bVarRd = *asSMBus[bSMBusCh].SMBusBData;
				bRdLength--;
				bVarRd++;						// point to next address of variable

				if (bRdLength == 1)
				{
					SET_MASK(*asSMBus[bSMBusCh].SMBusCTL, HOCTL_LABY);
				}
				else if (bRdLength == 0)						// write block transaction done
				{
					TR1 = 0;				// disable timer1
					TF1 = 0;				// clear overflow flag
					ET1 = 1;				// Enable timer1 interrupt
	...
	//*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xFD;		// Disable I2C_EN
	*asResetSMBusS[bSMBusCh].SMBusCTL2 &= 0xF5;		// Disable I2C_EN & I2C_SW_EN
	*asResetSMBusS[bSMBusCh].SMBusCTL2 |= 0x10;							// Enable SMBus Data Time out Enable
	*asSMBus[bSMBusCh].SMBusSTA = 0xFE;		// clear bits
